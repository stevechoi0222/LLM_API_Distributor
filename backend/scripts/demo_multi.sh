...
echo "==> Importing sample data (re-usable)"
...
echo "==> Creating run (OpenAI + Gemini + Perplexity)"
RUN_RESP=$(curl -s -H "x-api-key: $KEY" -H "Content-Type: application/json" \
  -d "{
    \"campaign_id\": \"$CAMPAIGN_ID\",
    \"providers\": [
      {\"name\":\"openai\",\"model\":\"gpt-4o-mini\",\"temperature\":0},
      {\"name\":\"gemini\",\"model\":\"gemini-1.5-flash\",\"temperature\":0},
      {\"name\":\"perplexity\",\"model\":\"llama-3.1-sonar-small-128k-online\",\"temperature\":0}
    ],
    \"prompt_version\": \"v1\",
    \"concurrency\": 6,
    \"rate_limits\": {
      \"openai\":{\"qps\":2,\"burst\":4},
      \"gemini\":{\"qps\":2,\"burst\":4},
      \"perplexity\":{\"qps\":1,\"burst\":2}
    }
  }" "$API/api/v1/runs")
echo "$RUN_RESP" | jq .
...
for i in {1..90}; do
  ...
  echo "[Poll $i/90] status=$STATUS pending=$PENDING running=$RUNNING succeeded=$SUCCEEDED failed=$FAILED"
...
for i in {1..30}; do
  ...
  if [[ "$STATUS" == "completed" && -n "$FILE_URL" ]]; then
    echo ""
    echo "=========================================="
    echo "✓ MULTI-PROVIDER DEMO COMPLETED"
    echo "=========================================="
    echo "XLSX created at: $FILE_URL"
    echo "Run ID: $RUN_ID"
    echo "Export ID: $EXPORT_ID"
    echo ""
    echo "The XLSX contains results from:"
    echo "  - OpenAI (gpt-4o-mini)"
    echo "  - Gemini (gemini-1.5-flash)"
    echo "  - Perplexity (llama-3.1-sonar-small)"
    echo "=========================================="
    exit 0
  fi
